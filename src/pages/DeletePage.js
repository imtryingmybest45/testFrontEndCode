...
function DeletePage(){
    ...
    const yesClick = (movie) => {
        ...
        //axios.post('http://localhost:8080/deleteEndpoint',movie)
        axios.post('https://testhelpme-cfh4afcpdreacnh8.canadacentral-01.azurewebsites.net/deleteEndpoint',movie)
    ...
    <div>
      ...
      {!tfTitle && <ul style={{listStyleType: 'none',  padding: "0", margin: "0"}}>
        {sortedEntries.map(([key,value])=><li key={value.movieId}><Link key={value.movieId} to={'/Delete'} onClick={()=>handleClick(value)}>{value.name}</Link></li>)}
      </ul>}
      {tfTitle && <p>{mess}</p>}
